...
const AddProduct = ({ initialValues, ...props }: any) => {
	...
	const [upadateProduct] = useMutation(UPDATE_PRODUCT);
	...
	const updateProductHandler = useCallback(async () => {
		try {
			...
			const result = await upadateProduct({
				variables: {
					input: insertProductData,
				},
			});
	...
	if (device === 'mobile') {
		return (
			<div id="add-product-page-mobile">
							...
							<Stack className="upload-box">
								...
								>
									...
									<input
										...
										type="file"
										hidden={true}
										onChange={uploadImages} //onChange bo'laganda uploadImages ishga tushadi
										multiple={true}
