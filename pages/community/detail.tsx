...
const CommunityDetail: NextPage = ({ initialInput, ...props }: T) => {
	...
	const updateButtonHandler = async (commentId: string, commentStatus?: CommentStatus.DELETE) => {
		try {
			...
			if (!updateData?.commentContent && !updateData?.commentStatus) throw new Error('Provide a comment to update!');
			...
			await sweetMixinErrorAlert(errr.message);
		} finally {
